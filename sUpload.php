    ...
    if(isset($_FILES["photo"]) && $_FILES["photo"]["error"] == 0){
        ...
        if(in_array($ext, $allowed)){
            ...
            if(file_exists("files/" . $FNameClean3)){
                ...
                include 'db1.php';
                $sql = "INSERT INTO app_send_file (date,name,size,type,ip) VALUES ('$x_date1', '$FNameClean3','$filesizeR','$filetype', '$ipy1')";                
                if ($conn->query($sql) === TRUE) {
                    echo "<br><br><textoRes>[[ Dados gravados no SQL ]]</textoRes>";
                } else {
                    echo "<textoerro>[ Error 22: " . $sql . " ]</textoerro>" . $conn->error;
                }
                $conn->close();                                              
            } 
        } else{
